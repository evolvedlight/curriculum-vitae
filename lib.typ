...
#let technologies-line(technologies: []) = {
  set block(below: 8pt, above: 1pt)
  table(
    columns: (1fr, 5fr),
    stroke: none,
  ...
  for entry in socials {
    ...
    if type(value) == str {
      if (key == "address") {
        ...
        socialsList.push(permit-social("address-card", value))
      } else {
...
#let cv-entry-multiline(
  ...
  technologies-line(technologies: technologies)
}
